...
resource "null_resource" "curlurl" {
  ...
  provisioner "local-exec" {
	...
	curl --location --request POST 'https://support-center.cloud.ibm.com/case-management/v1/cases' --header 'Content-Type: application/json' --header 'Authorization: Bearer $TOKEN' --data-raw '{ "type": "technical", "subject": "Case subject API Test",
  "description": "Case description API to Test", "severity":4, "offering": {"name": "Virtual Server for Classic", "type": {"group": "crn_service_name", "key": "virtual-server-group", "kind": "iaas", "id": "virtual-server-group"}}, "resources": [{"crn": "120473408"}]}'
    EOF
  }
}
